...
- (void) buildTable
{
    ...
    T_BIKE_INFO     *tbikeinfo;
    tbikeinfo = [[EAController sharedController] getBikeInfoPtr];
    
    
    unsigned int yearmodel;
    sscanf((char*)&tbikeinfo->VINyear,"%d",&yearmodel);
    
    
    printf("\n");
    self.dtcdict = nil;
    //[[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [year:%d model:%@]",yearmodel, model ]  ];
    
    
    if ( self.dtcdict == nil )
    {
        self.dtcdict = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"dtc_generic" ofType:@"plist"]];
    }
    ...
    dtcptr  = [[EAController sharedController] getDTCptr];
    text = [ NSString stringWithFormat:@"%s", dtcptr->error];
    ...
    [[EAController sharedController] sendUsbMessageToServer: [NSString stringWithFormat:@"FP3DTCViewController :: buildTable :: [dtccount:%d, dtc:%@]", (unsigned int)text.length, text]  ];
    ...
    if ( listcount >=1 )
    {
        for ( int ii=0; ii<listcount; ii++ )
        {
            ...
            const char *msg2 = [listDtc[ii] UTF8String];
            char buf[12];
            char *msg1;
            ...
            text = [NSString stringWithFormat:@"%s",buf];
            listOfIparsedfields = [text componentsSeparatedByString:@"."];
            
            
            if ( [listOfIparsedfields[1] isEqualToString:@"00"] == TRUE )
            {
                //field = listOfIparsedfields[0];
                text = [NSString stringWithFormat:@"%@",listOfIparsedfields[0]];
            }
            else
            {
                //text = [NSString stringWithFormat:@"%@,%@",listOfIparsedfields[0],listOfIparsedfields[1]];  // API crashes, so remove the comma between arguments.
                text = [NSString stringWithFormat:@"%@",listOfIparsedfields[0]];
                
            }
            
            //[[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [code:%@]",field ]  ];
            
            //[listOfItems addObject:text];
            
            //Add object to table Array
            tmpCellData = [[TCCellData alloc]initWithTroubleCode:text troubleCodeDescription:[self getTroubleCodeDescription:text]];
    ...
    else
    {
        ...
        [[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [ECM count:%d]",(unsigned int)listcount ]  ];
    ...
    dtcptr  = [[EAController sharedController] getDTCptr];
    text = [ NSString stringWithFormat:@"%s", dtcptr[1].error];
    
    [[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [BCM dtc:%@ %d]",text, (unsigned int)text.length ]  ];
    ...
    if ( listcount >=1 )
    {
        for ( int ii=0; ii<listcount; ii++ )
        {
            ...
            {
                //text = [NSString stringWithFormat:@"%@,%@",listOfIparsedfields[0],listOfIparsedfields[1]];
                text = [NSString stringWithFormat:@"%@",listOfIparsedfields[0] ];
    ...
    else
    {
        ...
        [[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [BCM count:%d]", (unsigned int)listcount ]  ];
    ...
    dtcptr  = [[EAController sharedController] getDTCptr];
    text = [ NSString stringWithFormat:@"%s", dtcptr[2].error];
    
    [[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [ABS dtc:%@ %d]",text, (unsigned int) text.length ]  ];
    ...
    else
    {
        ...
        [[EAController sharedController] sendUsbMessage: [NSString stringWithFormat:@"dtc - buildTable [ABS count:%d]",(unsigned int)listcount ]  ];
    ...
    field = [NSString stringWithFormat:@"%s",dtcptr->msg];
    //text = [field stringByReplacingOccurrencesOfString:@"_" withString:@" "];
    ...
    NSString *displayLocaleString;
    displayLocaleString =  NSLocalizedStringFromTableInBundle(field, nil, langBundle, nil);
    //NSLog(@"DTC msg: %@",displayLocaleString);
    [self.uilblDTCWarning setText:displayLocaleString];
    [self.uilblDTCWarning setTextColor:[[EAController sharedController] getActiveTextLabelTextColor]];
    
    
    T_DEVSTATE      *t_devstate;
    t_devstate = [[EAController sharedController] getDevStatePtr];
    
    if ( t_devstate->bAppDemoMode)
    {
        [self.notificationView hide:NO];
        [self updateStatusNotification:[NSString stringWithFormat:@"%@ - %@", localeDemoMode , displayLocaleString]:0 ];
    }
    else
    {
        [self.uilblDTCWarning setText:@""];
        [self updateStatusNotification:displayLocaleString:0];
    }
    [_uitableviewDTCcode setHidden:FALSE];
